...
from .utils import (
    load_config, setup_logging, get_device, tensor_to_pil, 
    pil_to_tensor, save_image_tensor, compute_lpips_score
)
...
def load_universal_params(params_path: str) -> dict:
    ...
    if not Path(params_path).exists():
        logger.warning(f"Universal params not found: {params_path}. Using default values.")
...
def main():
    ...
    parser.add_argument("--output", type=str, help="Output path (auto-generated if not provided)")
    parser.add_argument("--target-text", type=str, help="Target text (uses OCR if not provided)")
