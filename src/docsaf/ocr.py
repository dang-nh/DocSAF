...
def ocr_read(image: np.ndarray, lang: str = "en", backend: str = "easyocr") -> str:
    ...
    if backend == "easyocr":
        ...
        raise ValueError(f"Unsupported OCR backend: {backend}. Use 'easyocr' or 'paddleocr'")
...
def ocr_confidence(image: np.ndarray, lang: str = "en", backend: str = "easyocr") -> List[Tuple[str, float]]:
...
def _easyocr_confidence(image: np.ndarray, lang: str = "en") -> List[Tuple[str, float]]:
    ...
    try:
        results = reader.readtext(image, detail=1)  # detail=1 returns bbox, text, confidence
...
def _paddleocr_confidence(image: np.ndarray, lang: str = "en") -> List[Tuple[str, float]]:
