...
import torch
from transformers import AutoTokenizer, LayoutLMv2ForTokenClassification, AutoProcessor
...
# def perturb_bbox(bbox, max_offset=20):
#     x1, y1, x2, y2 = bbox
#     x1 = max(0, x1 + random.randint(-max_offset, max_offset))
#     y1 = max(0, y1 + random.randint(-max_offset, max_offset))
#     x2 = max(0, x2 + random.randint(-max_offset, max_offset))
#     y2 = max(0, y2 + random.randint(-max_offset, max_offset))
...
def infer(image_path, json_path, output_folder):
    ...
    sorted_form = sorted(form, key=lambda x: get_sorted_boxes(x["box_line"]), reverse=False)
    words, bboxes, labels_char = text_to_words(sorted_form)
    token_list = []
    bbox_list = []
    label_list = []
    for box, word, label in zip(bboxes, words, labels_char):
        tokens = tokenizer.tokenize(word)
        num_tokens = len(tokens)
        token_list.extend(tokens)
        bbox_list.extend([box] * num_tokens)
        label_list.extend([label] * num_tokens)
