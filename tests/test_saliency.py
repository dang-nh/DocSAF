...
class MockEmbedder:
    ...
    def __init__(self, embed_dim=512, device="cpu"):
        ...
        self.device = device
        # Simple linear layers to create gradient flows
        self.image_proj = torch.nn.Linear(3, embed_dim).to(device)
        self.text_proj = torch.nn.Linear(1, embed_dim).to(device)

    def image_embed(self, image):
        # Create embedding that depends on image input for proper gradients
        batch_size = image.shape[0]
        # Global average pool to get per-channel averages
        img_features = image.mean(dim=[2, 3])  # (B, 3)
        emb = self.image_proj(img_features)
        return torch.nn.functional.normalize(emb, dim=-1)

    def text_embed(self, texts):
        # Create embedding that varies with text (simple hash-based)
        batch_size = len(texts)
        # Simple text feature based on text length and hash
        text_features = torch.tensor(
            [float(hash(text) % 1000) / 1000.0 for text in texts], device=self.device
        ).unsqueeze(-1)
        emb = self.text_proj(text_features)
        return torch.nn.functional.normalize(emb, dim=-1)
...
class TestGradientSaliency:
    ...
    def test_saliency_normalization(self):
        ...
        _, saliency_norm = compute_gradient_saliency(
            embedder, x, "test", normalize=True
        )

        # Test without normalization
        _, saliency_raw = compute_gradient_saliency(
            embedder, x, "test", normalize=False
        )
    ...
    def test_saliency_monotonicity(self):
        ...
        x_blur = kornia.filters.gaussian_blur2d(
            x_orig.detach(), (kernel_size, kernel_size), (sigma, sigma)
        )
...
class TestMultiScaleSaliency:
    ...
    def test_multi_scale_basic(self):
        ...
        avg_alignment, avg_saliency = multi_scale_saliency(embedder, x, text, scales)
