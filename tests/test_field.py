...
from src.docsaf.field import (
    apply_field, apply_field_safe, field_with_smoothing,
    compute_tv_loss, validate_field_params, field_stats
)
...
class TestFieldApplication:
    ...
    def test_field_monotonic_lpips(self):
        ...
        for i in range(1, len(lpips_scores)):
            assert lpips_scores[i] >= lpips_scores[i-1] - 1e-4, \
...
class TestFieldStats:
    ...
    def test_field_stats_computation(self):
        ...
        expected_keys = ["l2_norm", "linf_norm", "mean_abs_diff", "max_abs_diff", "affected_pixels"]
...
class TestFieldBoundaryConditions:
    ...
    def test_extreme_alpha_values(self):
        ...
        # Very small alpha (should be close to identity)
        x_adv_small = apply_field(x, S, alpha=1e-6, radius=radius)
        assert torch.allclose(x_adv_small, x, atol=1e-3)
        
        # Large alpha (should create significant difference)
        x_adv_large = apply_field(x, S, alpha=10.0, radius=radius)
        diff = torch.norm(x_adv_large - x)
        assert diff > 0.1  # Should create noticeable change
