...
def eot_light_albumentations(
    ...
    try:
        ...
        logger.warning(
            "Albumentations not available, falling back to OpenCV implementation"
        )
    ...
    transforms = A.Compose(
        [
            A.ImageCompression(
                quality_lower=jpeg_q_min, quality_upper=jpeg_q_max, p=eot_prob
            ),
            A.RandomScale(scale_limit=(resize_min - 1.0, resize_max - 1.0), p=eot_prob),
    ...
    try:
        ...
        logger.warning(
            f"Albumentations transform failed: {e}. Returning original image."
        )
        return x
    ...
    try:
        ...
        if len(img1.shape) == 3:
            return structural_similarity(
                img1, img2, multichannel=True, channel_axis=2, data_range=255
            )
        else:
