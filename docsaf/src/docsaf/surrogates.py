...
class CLIPEmbedder:
    ...
    def __init__(self, model_name: str = "ViT-L-14", pretrained: str = "laion400m_e32", device: str = "cuda"):
        import open_clip
...
def load_clip_embedder(name: str = "ViT-L-14@336", device: str = "cuda") -> CLIPEmbedder:
...
def load_blip2_embedder(name: str = "blip2-opt-2.7b", device: str = "cuda") -> BLIP2Embedder:
...
def load_embedder(spec: str, device: str = "cuda") -> ImageTextEmbedder:
    ...
    """
    if not ":" in spec:
        raise ValueError(f"Invalid embedder spec: {spec}. Use 'openclip:model' or 'hf:model'")
    ...
    if backend == "openclip":
        ...
        raise ValueError(f"Unsupported embedder backend: {backend} with model: {model_name}")