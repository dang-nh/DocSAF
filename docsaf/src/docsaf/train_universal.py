...
from .utils import (
    load_config, setup_logging, get_device, set_random_seeds,
    pil_to_tensor, get_image_files, ensure_dir, compute_lpips_score
)
...
def train_universal_parameters(
    ...
    while step < steps:
        ...
        with torch.no_grad():
            _, saliency_maps = compute_batch_saliency(
                primary_embedder, images, texts
            )
            ...
            try:
                with torch.no_grad():
                    ...
                    x_adv_sample = apply_field(
                        images[:1], saliency_maps[:1], 
                        alpha, torch.nn.functional.softplus(radius)
                    )
                    lpips_score = compute_lpips_score(images[:1], x_adv_sample, device=device)
            except:
                lpips_score = 0.0
            
            logger.info(f"Step {step:>5}: Loss={total_loss.item():.4f}, "
        ...
        history["radius_values"].append(float(torch.nn.functional.softplus(radius).item()))
        history["alignment_losses"].append(float(loss_components["alignment_loss"].item()))
