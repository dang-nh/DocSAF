...
def alignment_collapse_loss(
    ...
    if len(texts) != x_adv.shape[0]:
        raise ValueError(f"Batch size mismatch: {len(texts)} texts, {x_adv.shape[0]} images")
...
def targeted_alignment_loss(
    ...
    """
    if len(original_texts) != len(target_texts) or len(original_texts) != x_adv.shape[0]:
    ...
    for embedder in embedders:
        ...
        orig_align = (F.normalize(img_emb, dim=-1) * F.normalize(orig_txt_emb, dim=-1)).sum(dim=-1)
        ...
        target_align = (F.normalize(img_emb, dim=-1) * F.normalize(target_txt_emb, dim=-1)).sum(dim=-1)
...
def docsaf_objective(
    ...
    if targeted_texts is not None:
        ...
        alignment_loss = alignment_collapse_loss(
            embedders, x_adv_eot, texts
        )
...
def validate_objective_inputs(
    ...
    if saliency_maps.shape != (images.shape[0], 1, images.shape[2], images.shape[3]):
        raise ValueError(f"Saliency shape mismatch: expected {(images.shape[0], 1, images.shape[2], images.shape[3])}, got {saliency_maps.shape}")
